...
class UpdateSeoChapters extends Command
{
    ...
    public function handle()
    {
        ...
        // Do some thing
        do {
            $midModel = Chapters::where('sticky', Chapters::STATUS_ON)->orderBy('id')->take(10)->get(['id', 'name']);
            foreach ($midModel as $chapter) {
                ...
                $data['seo_keywords']    = (strlen($keywords) < 191) ? $keywords : $chapter->name.', '.$chapter->name.' tiếng việt';
                $data['sticky']          = Chapters::STATUS_OFF;
                $chapter->update($data);
