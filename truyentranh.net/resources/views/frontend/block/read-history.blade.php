    ...
    <div class="block-history-read">
      <p class="save-manga">
        <a role="button" data-toggle="collapse" href="#collapsehistory" aria-expanded="true" aria-controls="collapsehistory">
          <img src="{{ asset(PATH_IMAGE_FRONTEND . 'clockfix.png') }}" alt="clock"> Xem lịch sử đọc truyện của bạn
        </a>
      </p>
    </div>
      ...
      @if(count($book_history) > 0)
        ...
        <div class="media media-smalltop">
          <div class="media-left media-topfix">
            <a href="{{ route('front.books.show', ['book_slug' => $book->slug]) }}" title="{{ $book->name }}"><img src="{!! asset(PATH_IMAGE_THUMBNAIL_BOOK.$book->image) !!}" alt="{{ $book->name }}" class="media-object"/></a>
          </div>
          <div class="media-body">
            <h4 class="manga-newest">
              ...
              <a href="{{ route('front.books.showdetail', ['chapter_slug' => $book->chapter_slug]) }}" title="{{$book->chapter_name}}"><strong class="chapter">  ({{ ucfirst($book->chapter_name) }})</strong> </a>
            </h4>
            <p class="description">
