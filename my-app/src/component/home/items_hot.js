...
class ItemsHot extends Component {
	render() {
		let list_chapters = this.props.list_chapter.map((chapter, index) => {
			return <li key={chapter.id}><a className="latest-chap" href={chapter.url} target="_blank" title={chapter.title}>{chapter.title}</a></li>
		});
		return (
			<div className="col-xs-12 col-md-6">
				<div className="media mainpage-manga mt-0">
					...
					<div className="media-body">
						<h4 className="manga-newest"><a href={this.props.url} title={this.props.title} data-tt="#truyenmoi-31347">{this.props.children}</a></h4>
						<div className="row">
