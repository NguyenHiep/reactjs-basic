...
class Header extends React.Component {
  ...
  render() {
    let elements_post_stick = this.state.posts_stick.map((post_slick, index) => {
      return <div className="post-stick" key={post_slick.id}><a href={post_slick.url}
                                                                title={post_slick.title}>{post_slick.title} <img
        src="http://cdn.truyentranh.net/frontend/images/hot.gif"/></a></div>
    });
    return (
      <div>
              ...
              <nav className="navbar navbar-expand-lg navbar-light col-12">
                ...
                <div className="collapse navbar-collapse" id="navbarSupportedContent">
                  <ul className="navbar-nav mr-auto">
                    ...
                    <li className="nav-item"><Link to='/hay-nhuc-nhoi' className="nav-link" style={{color: 'red'}}>Hay
                      nhức nhói</Link></li>
                    <li>
                      <form className="form-inline my-2 my-lg-0">
                        <input className="form-control mr-sm-2" type="search" placeholder="Tìm kiếm truyện"
                               aria-label="Search"/>
